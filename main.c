...
/*   Updated: 2023/06/14 22:38:07 by hyungdki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void fdf_init(t_fdf *fdf, t_map *map)
{
	...
	fdf->img_ptr = T_NULL;
	fdf->img_addr = T_NULL;
	fdf->win_size_x = (int)(map->largest_abs_x_2d * 2 + 100);
	if (fdf->win_size_x > 1600)
		fdf->win_size_x = 1600;
	fdf->win_size_y = (int)(map->largest_abs_y_2d * 2 + 100);
	if (fdf->win_size_y > 1000)
		fdf->win_size_y = 1000;
	map->fd = -1;
}
...
void map_init(t_map *map)
{
	...
	map->col = 0;
	map->fd = -1;
	map->largest_abs_x_2d = -1;
	map->largest_abs_y_2d = -1;
}
...
void fdf()
{
	...
	if (map.row > map.col)
	{
		if (map.row < 40)
			...
			map.basic_len = 1000 / (map.row + 10) + 1;
	}
	else
	{
		if (map.col < 40)
			...
			map.basic_len = 1000 / (map.col + 10) + 1;
	}
	...
	while (++r < map.row)
	{
		...
		while (++c < map.col)
		{
			map.map[r][c].x = (map.row / 2) * -1 + r;
			map.map[r][c].y = (map.col / 2) * -1 + c;
			map.map[r][c].x_2d = (int)(((map.map[r][c].y - map.map[r][c].x) * ISO_X) * map.basic_len + 0.5);
			if (fabs(map.map[r][c].x_2d) > map.largest_abs_x_2d)
				map.largest_abs_x_2d = fabs(map.map[r][c].x_2d);
			map.map[r][c].y_2d = (int)(((map.map[r][c].x + map.map[r][c].y) * ISO_Y - map.map[r][c].z) * map.basic_len + 0.5);
			if (fabs(map.map[r][c].y_2d) > map.largest_abs_y_2d)
				map.largest_abs_y_2d = fabs(map.map[r][c].y_2d);
		}
	}

	fdf_init(&fdf, &map);
	map.midpoint_x = fdf.win_size_x / 2;
	map.midpoint_y = fdf.win_size_y / 2;
	...
	while (++r < map.row)
	{
		...
		while (++c < map.col)
		{
			map.map[r][c].x_2d += map.midpoint_x;
			map.map[r][c].y_2d += map.midpoint_y;
		}
	}
	...
	while (++r < map.row)
	{
		...
		while (++c < map.col - 1)
		{
			my_mlx_pixel_put(&fdf, map.map[r][c].x_2d, map.map[r][c].y_2d, map.map[r][c].color.color);
...
void draw_line(t_fdf *fdf, t_point p1, t_point p2)
{
	...
	dx = fabs(p1.x_2d - p2.x_2d);
	dy = fabs(p1.y_2d - p2.y_2d);
	if (dx == 0 && dy == 0)
		return;
	else if (dx == 0)
...
void draw_vertical_line(t_fdf *fdf, t_point p1, t_point p2)
{
	...
	y = syp.y_2d;
	color_box = calc_color(syp, byp, byp.y_2d - syp.y_2d - 1);
	idx = -1;
	while (++y < byp.y_2d)
		my_mlx_pixel_put(fdf, p1.x_2d, y, color_box[++idx].color);
	free(color_box);
}
...
void draw_horizontal_line(t_fdf *fdf, t_point p1, t_point p2)
{
	...
	x = sxp.x_2d;
	color_box = calc_color(sxp, bxp, bxp.x_2d - sxp.x_2d - 1);
	idx = -1;
	while (++x < bxp.x_2d)
		my_mlx_pixel_put(fdf, x, p1.y_2d, color_box[++idx].color);
	free(color_box);
}
...
void draw_line_slope_abs1(t_fdf *fdf, t_point p1, t_point p2)
{
	...
	x = sxp.x_2d;
	move = -1;
	if ((p2.x_2d - p1.x_2d) / (p2.y_2d - p1.y_2d) == 1)
		move = 1;
	color_box = calc_color(sxp, bxp, bxp.x_2d - sxp.x_2d - 1);
	idx = -1;
	while (++x < bxp.x_2d)
...
void draw_line_move_x(t_fdf *fdf, t_point p1, t_point p2)
{
	...
	while (++x < bxp.x_2d)
	{
		y = (int)(((p2.y_2d - p1.y_2d) / (p2.x_2d - p1.x_2d)) * (x - p1.x_2d) + p1.y_2d + 0.5);
		my_mlx_pixel_put(fdf, x, y, color_box[++idx].color);
	}
	free(color_box);
}
...
void draw_line_move_y(t_fdf *fdf, t_point p1, t_point p2)
{
	...
	while (++y < byp.y_2d)
	{
		x = (int)(((p2.x_2d - p1.x_2d) / (p2.y_2d - p1.y_2d)) * (y - p1.y_2d) + p1.x_2d + 0.5);
		my_mlx_pixel_put(fdf, x, y, color_box[++idx].color);
	}
	free(color_box);
}
...
void map_parsing3(t_map *map, char **split_result, int *row_idx)
{
	...
	split_idx = 0;
	while (split_result[split_idx] != T_NULL)
		split_idx++;
